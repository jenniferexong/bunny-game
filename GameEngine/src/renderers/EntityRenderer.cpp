...
void EntityRenderer::renderInstanced(const Environment& environment)
{
	...
	for (const auto& element : environment.getEntitiesInView()) {
		...
		prepareTexturedModel(model);
		const auto& batch = element.second;
		Print::val("batch", (int)batch.size());
		...
		Print::val("transformations", (int)model_matrix_data.size());
		Print::val("brightness", (int)model_brightness_data.size());
...
void EntityRenderer::loadColors(const std::unordered_set<weak_ptr<Entity>>& entities, vector<float>& float_data)
{
...
void EntityRenderer::loadTransformations(const std::unordered_set<weak_ptr<Entity>>& entities, vector<float>& float_data)
{
	for (const auto& e: entities) {
		glm::mat4 t_matrix = Maths::createTransformationMatrix(e.lock()->getPosition(), 
			e.lock()->getActualRotation(), e.lock()->getScale(), e.lock()->getAlignmentRotation());
