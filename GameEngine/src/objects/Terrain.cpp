...
vec3 Terrain::getNormalOfTerrain(float world_x, float world_z) const
{
	...
	vec3 answer(0.f), v1(0.f), v2(0.f), v3(0.f);

	/*
	// cases where x is 0 or 1, or z is 0 or 1
	if (fmod(x, 1.f) == 0.f || fmod(z, 1.f) == 0.f) {
		// on a corner of a triangle
		if (fmod(x, 1.f) == 0.f && fmod(z, 1.f) == 0.f) {
			answer = getNormal(grid_z, grid_x);
		}
		// top edge of grid square
		else if (z == 0.f) {
			vec3 n1 = getNormal(grid_z, grid_x);
			vec3 n2 = getNormal(grid_z, grid_x + 1);
			answer = normalize(n1 + n2);
		}
		// bottom edge of grid squre
		else if (z == 1.f) {
			vec3 n1 = getNormal(grid_z + 1, grid_x);
			vec3 n2 = getNormal(grid_z + 1, grid_x + 1);
			answer = normalize(n1 + n2);
		}
		// left edge of grid square
		else if (x == 0.f) {
			vec3 n1 = getNormal(grid_z, grid_x);
			vec3 n2 = getNormal(grid_z + 1, grid_x);
			answer = normalize(n1 + n2);
		}
		// right edge of grid square
		else if (x == 1.f) {
			vec3 n1 = getNormal(grid_z, grid_x + 1);
			vec3 n2 = getNormal(grid_z + 1, grid_x + 1);
			answer = normalize(n1 + n2);
		}
	}
	*/
	//else {
		...
		if (x < (1 - z)) {
			v1 = getPosition(grid_z, grid_x);
			v2 = getPosition(grid_z + 1, grid_x);
			v3 = getPosition(grid_z, grid_x + 1);
		}
		// right triangle
		else if (x > (1 - z)) {
			v1 = getPosition(grid_z + 1, grid_x + 1);
			v2 = getPosition(grid_z, grid_x + 1);
			v3 = getPosition(grid_z + 1, grid_x);
		}
		// calculate the normal of this triangle (face of the terrain)
		vec3 direction_1 = v2 - v1;
		vec3 direction_2 = v3 - v1;
		answer = normalize(cross(direction_1, direction_2));
	//}
	return answer;
}
...
Mesh Terrain::generate(const string& height_map)
{
	...
	for (int row = 0; row < vertex_count_; row++) { // row = z
		for (int col = 0; col < vertex_count_; col++) { // col = x
			positions_.push_back((float)col / ((float)vertex_count_ - 1) * size);
			float ht = calculateHeight(row, col, buffer);
			heights_.push_back(ht);
			positions_.push_back(ht);
			positions_.push_back((float)row / ((float)vertex_count_ - 1) * size);
