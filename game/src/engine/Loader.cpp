...
int Loader::loadToVao(const vector<float>& positions,
                      const vector<float>& texture_coords) {
    int vao_id = createVao();
...
InstancedMesh Loader::loadToVaoInstanced(const string& obj_name) {
    ...
    InstancedMesh mesh =
        InstancedMesh(vao_id, (int)data.indices.size(), data.face);
    mesh.setBoundingSphere(
        BoundingSphere(data.model_center, data.model_radius));

    int model_matrix_vbo =
        createInstancedVbo(AttributeLocation::ModelMatrixColumn1, 4, 16, 4);
    int brightness_vbo =
        createInstancedVbo(AttributeLocation::ModelBrightness, 1, 1, 0);
...
int Loader::loadCubeMap(std::vector<std::string> texture_names) {
    ...
    // order of textures for faces must be: right, left, top, bottom, back,
    // front
    for (int i = 0; i < 6; i++) {
        SkyboxTextureData data = SkyboxTextureData(texture_names.at(i));
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, data.width,
...
void Loader::storeInAttributeList(int attrib_num, int coord_size,
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * data.size(), data.data(),
                 GL_STATIC_DRAW);
    glVertexAttribPointer(attrib_num, coord_size, GL_FLOAT, GL_FALSE, 0,
                          nullptr);
 ...
 */
int Loader::createInstancedVbo(int start_location, int blocks, int num_elements,
                               int offset) {
    GLuint vbo_id;
    glGenBuffers(1, &vbo_id);
    ...
    for (int i = 0; i < blocks; i++) {
        glVertexAttribPointer(start_location + i, blocks, GL_FLOAT, GL_FALSE,
